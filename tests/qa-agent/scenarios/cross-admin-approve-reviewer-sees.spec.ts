...
export async function run(ctx: ScenarioCtx) {
  ...
  // Active count parity
  const peers = await sql<{ id: string; full_name: string; active_cases_count: number }>(`SELECT id, full_name, active_cases_count FROM reviewers WHERE active_cases_count > 0 LIMIT 10`).catch(() => null);
  if (peers) {
