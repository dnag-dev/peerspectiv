...
review_cases (id uuid, batch_id → batches, provider_id → providers, peer_id → peers, company_id → companies, assigned_at, due_date, status, encounter_date date, chart_file_name, specialty_required, created_at, updated_at)
review_results (id uuid, case_id → review_cases, peer_id → peers, overall_score int, quality_score int, deficiencies jsonb, submitted_at, time_spent_minutes int, narrative_final, created_at)
peer_payouts (id uuid, peer_id → peers, period_start date, period_end date, unit_type, units numeric, rate_amount numeric, amount numeric, status, approved_at, paid_at, created_at)
